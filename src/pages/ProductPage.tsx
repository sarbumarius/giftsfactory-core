...
const ProductPage = () => {
  ...
  const getCategorySlug = (category?: { slug?: string; slug_en?: string }) =>
  ...
  const getProductSlug = (product?: { slug?: string; slug_en?: string }) =>
    locale === 'en' ? product?.slug_en ?? product?.slug ?? '' : product?.slug ?? '';
  ...
  const buildPersonalizarePayload = () =>
    personalizareFields
      .map((field) => {
        ...
        return {
          name: field.name,
          label: field.label || t('product.option'),
          type: field.type,
          value,
          file,
  ...
  const handleAddOrOpenPersonalizare = () => {
    ...
    if (window.rybbit?.event) {
      ...
      window.rybbit.event('AddToCart', {
        ...
        product: displayTitle || t('product.unknown'),
  ...
  if (loading) {
    return (
      ...
      <MobileProductHeader
        title={`${t('common.loading')}${loadingDots}`}
        onBack={() => navigate(-1)}
  ...
  if (error || !data) {
    return (
      ...
      <MobileProductHeader
        ...
        onWishlistClick={() => navigate(withLocalePath('/wishlist'))}
      />
        <p className="text-destructive">{t('product.loadError')}</p>
      </div>
    );
  }
  ...
  const goToCategory = (category: { slug?: string; slug_en?: string }) => {
    if (!category?.slug) return;
    setCurrentSlug(category.slug);
    const targetSlug = getCategorySlug(category);
  ...
  return (
      <div className="min-h-screen bg-white pb-4">
              ...
              >
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-foreground">
                      {t('product.realPhotosPrefix', { count: photoItems.length })}
                    </p>
                    ...
                    {totalPhotoPages > 1 && (
                      <>
                        <button
                            ...
                            aria-label={t('product.reviewPrev')}
                        >
                        ...
                        <button
                            ...
                            aria-label={t('product.reviewNext')}
                        >
          ...
          {locale !== 'en' && data.descriere_scurta && (
            <div className="mt-2">
              <div
              ...
              >
                {showFullShortDesc ? t('product.showLess') : t('product.showMore')}
              </button>
            </div>
          )}
              ...
              <label htmlFor="preview-toggle" className="grid cursor-pointer grid-cols-7 items-start gap-3 rounded-xl border border-[#e9e6e3] bg-white p-3 shadow-sm">
                ...
                <span className="col-span-5 flex flex-col">
                  <span className="mb-1 font-semibold leading-tight">
                    {t('product.previewToggle')} <span className="text-sm font-normal text-darker-gray">(+15 lei)</span>
                  </span>
                  <span className="text-xs leading-snug text-darker-gray">
                    {t('product.previewToggleHint')}
                  </span>
                </span>
              </label>
            )}
                    ...
                    {field.type === 'checkboxes' && (
                      <div className="space-y-2">
                        {field.options?.length ? (
                          ...
                          <p className="text-xs text-muted-foreground">{t('product.noOptions')}</p>
                        )}
                      </div>
                    )}
                  </div>
          ...
          {personalizareFields.length > 0 && (
            <div className="-mt-2 rounded-t-2xl border border-b-0 border-[#d69149] bg-white px-2 py-2">
              <div className="grid grid-cols-[30%_70%] items-center">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">{t('product.adaptable')}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t('product.adaptableHint')}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex gap-2">
                  ...
                  >
                      {showPersonalizare || personalizareFields.length === 0 ? t('product.addToCart') : (
                        <span className="flex items-center justify-center gap-2">
                          {t('product.startPersonalization')}
      ...
      <section className="relative isolate mt-6">
        ...
        <div className="space-y-2 px-4 py-4">
            <div className="rounded-md">
                ...
                >
                    <span className="text-lg font-semibold text-[#6844c1]">{t('product.details')}</span>
                ...
                >
                    <div className="py-2 text-sm">
                        {orderedAttributes.length > 0 ? (
                            ...
                            <p className="text-muted-foreground">{t('product.detailsUnavailable')}</p>
                        )}
                    </div>
                </div>
            </div>
          ...
          {isLedProduct && (
            <>
              <div className="rounded-md">
                ...
                >
                  <span className="text-lg font-semibold text-[#6844c1]">{t('product.ledBaseInfo')}</span>
                  <div className="rounded-full p-1 transition-all">
          ...
          {isMusicProduct && (
            <>
              <div className="rounded-md">
                ...
                >
                  <span className="text-lg font-semibold text-[#6844c1]">{t('product.speakerInfo')}</span>
                  <div className="rounded-full p-1 transition-all">
          ...
          <div className="rounded-md">
            ...
            >
              <span className="text-lg font-semibold text-[#6844c1]">{t('product.description')}</span>
              <div className="rounded-full p-1 transition-all">
            ...
            >
              <div className="py-2 text-sm text-muted-foreground prose max-w-none">
                {(locale === 'en' ? data.descriere_en : data.descriere) ? (
                  <div
                    dangerouslySetInnerHTML={{
                      __html: locale === 'en' ? data.descriere_en || '' : data.descriere,
                    }}
                  />
                ) : (
                  <p>{t('product.descriptionUnavailable')}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-md">
            ...
            >
              <span className="text-lg font-semibold text-[#6844c1]">{t('product.quantityDiscounts')}</span>
              <div className="rounded-full p-1 transition-all">
            ...
            >
              <div className="py-2">
                <div className="rounded-xl border border-amber-100 bg-amber-50/60 px-4 py-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">{t('product.unitPrice')}</p>
          ...
          <div className="rounded-md">
            ...
            >
              <span className="text-lg font-semibold text-[#6844c1]">{t('product.graphicPreview')}</span>
              <div className="rounded-full p-1 transition-all">
          ...
          <div className="rounded-md">
            ...
            >
              <span className="text-lg font-semibold text-[#6844c1]">{t('product.shippingReturn')}</span>
              <div className="rounded-full p-1 transition-all">
          ...
          <span className="font-bold mb-2 w-full justify-center flex font-serif text-xl mt-4">{t('product.benefitsIncluded')}</span>
          <div className="bg-card shadow-md m-4 mb-0 rounded-t-2xl ">
            ...
            <span className="flex items-center font-bold">
                <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 2" viewBox="0 0 202.63 222.09"
                    ...
                    d="M179.67 143.22c-4.76 1.83-8.23 6.48-11.23 10.48-3.14-4.2-6.52-8.21-11.23-10.72 4.53-2.9 8.28-6.73 11.23-11.22 2.75 4.98 6.79 8.14 11.23 11.46M69.87 10.48l-.31 1.02c-4.77 2.11-7.76 6.54-10.92 10.44-2.78-4.2-6-8.41-10.73-10.48l.31-1.03C52.76 8.11 55.93 4.17 58.64 0c3.2 3.89 6.34 8.7 11.23 10.48"></path></g></svg>{t('benefits.premiumMaterials')}</span>
                  ...
                  d="M195.75 102.33v28.45l-28.8-28.35-62.28-.11c-5.8-.59-10.98-5.28-11.25-11.21l.36-82.52c1.01-4.16 6.21-8.1 10.39-8.57L203.51 0c7.14.65 11.18 5.83 11.73 12.73 1.96 24.85-1.49 52.25-.03 77.37-.39 4.3-2.7 8.82-6.63 10.84-.43.22-3.4 1.39-3.6 1.39z"></path></g></svg>{t('benefits.support')}</span><span
                  className="flex items-center font-bold"><svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 2"
                                                               viewBox="0 0 449.29 433.53"
                                                               className="mr-2 h-6 w-6 md:h-8 md:w-8"><g
                  data-name="Layer 2"><path
                  d="M389.36 104.66v162.72c10.07-7.6 20.62-15.1 32.27-20.14 9.17-3.96 21.38-8.94 26.53 3.76 5.26 12.99-9.15 17.86-16.89 24.85-6.78 6.13-12.72 13.3-19.49 19.46 53.75 109.98-96.56 194.43-162.68 91.99l-43.45 21.83V190l183.7-85.35Zm43.92 152.75c-12.61 3.76-24.31 11.71-34.83 19.56-24.36 18.16-45.82 41.24-65.5 64.27l-15.42-22.44-27.99 7.98 39.88 52.94 2.09-.15c26.05-40.13 54.52-79.15 89.27-112.21zm-110.01 3.28c-79.96 6.62-100.5 115.88-29.8 150.99 70.66 35.08 144.1-41.36 105.88-109.87l-14.51 16.38c17.68 47.37-26.97 95.01-75.34 77.46-18.34-6.65-35.66-26.32-37.84-46.01-6.06-54.55 51.21-87.52 95.1-55.37l14.53-13.55c.38-2.18-9.83-8.55-11.98-9.76-13.32-7.52-30.78-11.53-46.04-10.26ZM184.7 409.14 0 314.8V104.66l184.7 85.36zM273.55 134.63c.51 2.33-.31 1.6-1.28 2.22-1.87 1.21-5.15 2.54-7.34 3.61-23.17 11.37-47.06 21.73-70.67 32.08L8.99 85.71l85.1-38.76zM380.37 85.71l-85.12 39.75L117.1 38.01l-1.29-2.22L192.67.27l3.26-.27z"></path></g></svg>{t('benefits.insurance')}</span>
              </div>
          </div>
          ...
          <div className="mx-4  overflow-hidden rounded-b-2xl border border-[#dfb695] bg-gradient-to-r from-[#db6f4a] to-[#db6f4a] px-4 py-3 shadow-card">
              <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-[#fff]">
                      {t('product.pointsText', { points: rewardPoints })}
                  </p>
              </div>
          </div>
        ...
        <div className="mt-3  bg-[#d0c6eb] px-3 py-3 hidden">
          ...
          <div className="relative mt-4 flex items-start text-[11px] font-semibold text-[#6844c1]">
            ...
            <div className="flex flex-1 flex-col items-center text-center">
              <span className="mt-1 leading-tight">{t('product.estimatedDelivery')}</span>
        ...
        <div className="mt-6 px-4">
          <div className="rounded-2xl border border-border bg-white p-4">
            <div className="text-center">
              <p className="text-lg font-semibold font-serif text-foreground">{t('product.customerReviews')} ({data.recenzii.length})</p>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {t('product.reviewsSummary', { total: data.recenzii.length, withImages: imageReviewCount })}
              </p>
            </div>
            ...
            <div id="review-write-card" className="mt-4 rounded-xl border border-[#e9e6e3] bg-[#f9f6f2] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-foreground">
                    {t('product.writeReviewReward')}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">{t('product.shareExperience')}</p>
                </div>
                <button
                  type="button"
                ...
                >
                  {showReviewForm ? t('product.reviewFormClose') : t('product.reviewFormOpen')}
                </button>
              </div>
              ...
              {showReviewForm && (
                <div className="mt-4 space-y-3">
                  ...
                  />
                  <textarea
                    placeholder={t('product.reviewMessagePlaceholder')}
      ...
      {data.categorii?.length > 0 && (
        <div className="mt-6 pl-4" id="product-categories">
          <p className="text-xl font-serif text-foreground">{t('product.maybeInterested')}</p>
            ...
            {data.categorii.map((category, index) => (
              ...
              >
                <button
                  ...
                  data-track-action={`A deschis categoria ${getCategoryTitle(category)} din pagina produs.`}
                    ...
                    >
                      {category.produse.map((produs) => {
                        ...
                        <button
                          key={produs.id}
                          type="button"
                          onClick={() => navigate(withLocalePath(`/produs/${getProductSlug(produs)}`))}
                          data-track-action={`A deschis produsul ${getProductTitle(produs)} din carusel categorie.`}
                        ...
                        >
                          <div className="relative aspect-[4/5] overflow-hidden rounded-t-2xl bg-muted">
                            ...
                            <img
                              ...
                              alt={getProductTitle(produs)}
                              className="h-full w-full object-cover"
                              loading="lazy"
                            />
                          </div>
                          <div className="px-3 pb-3 pt-3">
                            <p className="line-clamp-2 text-sm font-serif text-foreground">{getProductTitle(produs)}</p>
                          </div>
                        </button>
                      );
                      })}
      ...
      >
        <div className="relative flex w-full items-center justify-between gap-1 border-t border-[#e6dfd8] bg-white/95 px-3 py-2 shadow-lg backdrop-blur">
          ...
          <button
            ...
            aria-label={showPersonalizare || personalizareFields.length === 0 ? t('product.addToCart') : t('product.customize')}
          >
                      ...
                      {data.categorii?.length > 0 ? (
                          <div className="mt-4">
                              <div className="mt-2 space-y-2">
                                  {(showAllConfirmCategories ? data.categorii : data.categorii.slice(0, 3)).map((category) => (
                                      <button
                                          ...
                                          data-track-action={`A mers la categoria ${getCategoryTitle(category)} din confirmarea cosului.`}
                                      ...
                                      >
                                          <span className="w-full text-center">{getCategoryTitle(category)}</span>
                                          <img
                                               src={category.imagine}
                                               alt={getCategoryTitle(category)}
                              ...
                              {data.categorii.length > 3 && (
                                ...
                                >
                                  {showAllConfirmCategories ? t('product.showLessCategories') : t('product.showMoreCategories')}
                                </button>
                              )}
                          </div>
                      ) : (
